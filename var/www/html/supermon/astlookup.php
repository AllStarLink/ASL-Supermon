...
if ($_SESSION['sm61loggedin'] === true) {
    ...
    if (AMIlogin($fp, $config[$localnode]['user'],
            $config[$localnode]['passwd']) === false)  // Login to Asterisk manager
    {
    ...
    if ("$intnode" != "$node") {        // Do lookup by callsign
        ...
        if ($intnode > 80000 && $intnode < 90000) {  // Lookup by IRLP node number

            do_irlp_number_search($intnode);
        } else {
            if ($intnode > 3000000) {      // Lookup by echolink node number
