      ...
      if (count($nodes) > 0) {
        ...
        <SCRIPT>
          ...
          function OpenArchiveURL() {
            window.open('<?php if (!empty($ARCHIVE_URL)) {
                print "$ARCHIVE_URL";
            } ?>');
          }
        </SCRIPT>
        ...
        <input style="margin-top:5px;" type="button" class="submit" value="Configuration Editor"
...
foreach ($nodes as $node) {
    ...
    if (($info == "Node not in database") || (isset($config[$node]['hideNodeURL']) && $config[$node]['hideNodeURL'] == 1)) {
        ...
        if (isset($config[$node]['lsnodes'])) {
            ...
            if ((preg_match("/localhost/", $config[$node]['host'])) || ((preg_match("/127.0.0.1/",
                    $config[$node]['host'])))) {
    ...
    ?>
  <table class=gridtable id="table_<?php
  echo $node ?>">
    <colgroup>
      <col span="1">
      <col span="1">
